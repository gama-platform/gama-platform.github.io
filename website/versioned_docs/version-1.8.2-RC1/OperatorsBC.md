...
#### Possible uses: 
  * `container&lt;unknown,geometry>` **`closest_tob`** `geometry` --->  `geometry`
  * **`closest_tob`** (`container&lt;unknown,geometry>` , `geometry`) --->  `geometry` 

#### Result: 
Return the closest agent from a point !!
...
#### Examples: 
``` 
geometry var0 <- closest_tob(population, point) or list closest_tob geometry; // var0 equals Return the closest agent from a point !!
```
